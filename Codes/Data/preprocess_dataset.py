...
import sys
import time
from Libs.Utils.utils import print_dict, save_json_file
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        print("Example usage:")
        print("python Libs/Datasets/preprocess_dataset.py Twitter16 3\n")
        exit()
    ...
    if dataset in ['Twitter15', 'Twitter16']:
        ...
        # --------------------------
        # Input
        paths['raw'] = 'Input/raw_data/data/orig/rumor_detection_acl2017/'
        paths['raw_label'] = os.path.join(
            paths['raw'], dataset.lower(), 'label.txt')
        paths['raw_tree'] = os.path.join(
            paths['raw'], dataset.lower(), 'tree/')
        ...
        paths['timestamps_raw'] = 'Input/raw_data/data/timestamps/{}/timestamps_raw.txt'.format(
            dataset)
        paths['timestamps'] = 'Input/raw_data/data/timestamps/{}/timestamps.txt'.format(
            dataset)
        paths['sequential_snapshots'] = 'Input/raw_data/data/timestamps/{}/sequential_snapshots_{:02}.txt'.format(
            dataset, snapshot_num)
        paths['temporal_snapshots'] = 'Input/raw_data/data/timestamps/{}/temporal_snapshots_{:02}.txt'.format(
            dataset, snapshot_num)
        print_dict(paths)
        ...
        temporal_info = raw_tree_to_timestamps(
            paths['raw_tree'], paths['timestamps'])
        ...
        paths['timestamps'] = './data/timestamps/{}/timestamps.txt'.format(
            dataset)
        paths['sequential_snapshots'] = './data/timestamps/{}/sequential_snapshots_{:02}.txt'.format(
            dataset, snapshot_num)
