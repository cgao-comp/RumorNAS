...
from Libs.Utils.utils import ensure_directory, load_json_file, print_dict
from preprocess_dataset import load_resource_labels, load_resource_labels_weibo
...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        print("Example usage:")
        print("python Libs/Datasets/prepare_snapshots.py Twitter16 sequential 3")
        exit()
    ...
    if dataset in ['Twitter15', 'Twitter16']:
        paths['resource_label'] = 'Input/raw_data/resources/{0}/{0}_label_all.txt'.format(
            dataset)
        paths['resource_tree'] = 'Input/raw_data/resources/{0}/data.TD_RvNN.vol_5000.txt'.format(
            dataset)
        paths['timestamps'] = 'Input/raw_data/data/timestamps/{}/timestamps.txt'.format(
            dataset)
        paths['snapshot_index'] = 'Input/raw_data/data/timestamps/{}/{}_snapshots_{:02}.txt'.format(
            dataset, dataset_type, snapshot_num)
        paths['graph'] = 'Input/raw_data/data/graph/{0}/{1}_snapshot/'.format(
            dataset, dataset_type)
    elif dataset in ['Weibo']:
        paths['resource_label'] = 'Input/raw_data/resources/{0}/weibo_id_label.txt'.format(
            dataset)
        paths['resource_tree'] = 'Input/raw_data/resources/{0}/weibotree.txt'.format(
            dataset)
        paths['timestamps'] = 'Input/raw_data/data/timestamps/{}/timestamps.txt'.format(
            dataset)
        paths['snapshot_index'] = 'Input/raw_data/data/timestamps/{}/sequential_snapshots_{:02}.txt'.format(
    ...
    if dataset in {'Twitter15', 'Twitter16', }:
        id_label_dict, _ = load_resource_labels(paths['resource_label'])
        trees_dict = load_snapshot_trees(
            paths, id_label_dict, sequences_dict, snapshot_num)
    elif dataset in {'Weibo', }:
        id_label_dict, _ = load_resource_labels_weibo(paths['resource_label'])
        trees_dict = load_snapshot_trees_weibo(
            paths, id_label_dict, sequences_dict, snapshot_num)
